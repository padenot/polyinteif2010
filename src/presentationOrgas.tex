\orga{images/orgas/armand.jpg}{%
\paragraph{Armand :} Grand aigri de la troupe, il s'occupera de la sécurité au WEI ! Autant dire que c'est mal barré... Attention pas de blague avec lui ou tu en subiras les conséquences, parole d'ex co-TP !
}

\orga{images/orgas/billy.jpg}{%
\paragraph{Billy :} Le secrétaire de la GPL, infiltré dans l'équipe d'inté pour distiller des propos subversifs à propos de gnous et de manchots.
}

\orga{images/orgas/clement.jpg}{%
\paragraph{Clément :} Né avec un appareil photo dans la main, c'est le photographe officiel du départ', et tu ne verras jamais ses yeux qu'à travers un objectif.
}

\columnbreak
~\\

\orga{images/orgas/gael.jpg}{%
\paragraph{Gaël :} Actuel président de l'AEDI, c'est grâce à lui que le bureau tourne, mais il a néanmoins son petit péché mignon... Si ta petite sœur est mineure, ne la ramène jamais avec toi, te voilà prévenu.
}
\vspace{1cm}
\orga{images/orgas/guillaume.jpg}{%
\paragraph{Guillaume} Ancien prépa, il a enfilé ses bas résilles et son tailleur en prenant le poste de secrétaire de l'AEDI. Si tu ne dois retenir qu'une seule chose a propos de lui, c'est deux bras.
}
\orga{images/orgas/julian.jpg}{%
\vfill
\paragraph{Julian :} C'est le maître des jeux, il possède les dés, il a le pouvoir, et tu seras sous son contrôle tôt ou tard...
}
\clearpage
\orga{images/orgas/julien.jpg}{%
\paragraph{Julien :}  Le seul, l'unique... Le Resp' Inté ! Grand, beau, fort, musclé, intelligent, menaçant, c'est LUI le patron, et il a le précieux !
}
\orga{images/orgas/loic.jpg}{%
\paragraph{Loic :} Ancien prépa et glandeur né (ses co-TP témoigneront).
    Bonjourlesifs dit qu'il possède la plus grosse de tout IF mais c'est
    uniquement pour pas qu'on le perde lorsque les herbes sont trop hautes.
}
\orga{images/orgas/marc.jpg}{%
\paragraph{Marc :} Véritable dictionnaire à acronymes. C'est la seule personne parmi les orgas à pouvoir comprendre le sens réel et profond de cette phrase : «~\texttt{OMG LOL WTF STFU U SUXX L2P U NAAB GTFO !!!1}~»
}

\columnbreak

    \vfill
\orga{images/orgas/nicolas.jpg}{%
    \vfill
\paragraph{Nical :} Graphiste officiel de l'inté, ses dessins te feront rêver à
    d'autres lieux tandis que lui restera enchaîné à notre dominatrice Resp'
    semaine d'inté.
}
\orga{images/orgas/paul.jpg}{%
    \vfill
\paragraph{Paul :} Musicien libriste buvant du café à longueur de journée. Il
    trouve également le temps d'être le président de la GPL grâce à un secret
    que j'ose à peine te dévoiler... C'est une loutre !}
    \vfill
\orga{images/orgas/quentin.jpg}{%
    \vfill
\paragraph{Quentin :} Président du vice de l'AEDI... Vice président pardon. Peu de personnes ont essayé de le suivre côté alcool, et les pauvres ayant tenté l'expérience n'en sont pas ressortis indemnes.
}
    \vfill

\columnbreak

\orga{images/orgas/sandra.jpg}{%
\paragraph{Sandra :} Responsable de la semaine d'inté, elle règne sur ses fidèles escla... serva... compagnons avec grâce et fermeté.
}
\vspace{0.5cm}
\orga{images/orgas/sebastien.jpg}
\paragraph{Seb :} L'espion mi-orga mi-bizuth, plus fort que James Bond, il peut utiliser ses nombreux atouts pour se sortir de n'importe quelle situation... Sauf contre Chuck Norris.
\vfill
\columnbreak
\orga{images/orgas/vincent.jpg}
\paragraph{Vincent :} Barman de conviction, il te fera goûter à de délicieux cocktails. Surnommé Papy Bouli, autant par sa vitesse de déplacement que par son air ronchon, tu le reconnaîtras à ses «~Oh Marseille !~»
\orga{images/orgas/zaza.jpg}
\paragraph{Zaza :} Trésorière de l'AEDI, elle attend les soldes impatiemment avec son nouveau chéquier ! Responsable WEI, elle dirige le sexe fort d'une main de maître(sse).

\end{multicols}
\begin{center}
    \includegraphics[height=8cm]{images/stormtrooper_biere.png}
\end{center}
\newpage
\begin{multicols}{2}
